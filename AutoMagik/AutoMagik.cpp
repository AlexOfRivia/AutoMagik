...
void AutoMagik::updateManagerTables()
{
    ...
    //Update Cars Table
    //adding cars from db
    QString carUrl = "https://automagik-96e43-default-rtdb.europe-west1.firebasedatabase.app/automagik/cars.json?auth=" + firebase.getIdToken();
    QNetworkRequest carRequest((QUrl(carUrl)));
    carRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    QNetworkReply* carReply = firebase.getNetworkAccessManager()->get(carRequest);

    connect(carReply, &QNetworkReply::finished, &loop, &QEventLoop::quit);
    loop.exec();

    QByteArray carData = carReply->readAll();
    QJsonDocument carDoc = QJsonDocument::fromJson(carData);
    QJsonObject allCars = carDoc.object();

    for (const QString& key : allCars.keys()) {
        QJsonObject carData = allCars[key].toObject();
        if (carData["managerId"].toString() == firebase.m_uid) {
            Car w;
            w.setCarMake(carData["make"].toString().toStdString());
            w.setCarModel(carData["model"].toString().toStdString());
            w.setEngineType(carData["engineType"].toString().toStdString());
            w.setProductionYear(carData["year"].toInt());
            w.setCarMileage(carData["mileage"].toInt());
            w.setClientPhoneNumber(carData["phoneNumber"].toInt());
            w.setFirebaseKey(key.toStdString());
            cars.push_back(w);
        }
    }
    ...
    //Update Tasks Table
    //add tasks from db
    QString taskUrl = "https://automagik-96e43-default-rtdb.europe-west1.firebasedatabase.app/automagik/tasks.json?auth=" + firebase.getIdToken();
    QNetworkRequest taskRequest((QUrl(taskUrl)));
    taskRequest.setHeader(QNetworkRequest::ContentTypeHeader, "application/json");
    QNetworkReply* taskReply = firebase.getNetworkAccessManager()->get(taskRequest);

    connect(taskReply, &QNetworkReply::finished, &loop, &QEventLoop::quit);
    loop.exec();

    QByteArray taskData = taskReply->readAll();
    QJsonDocument taskDoc = QJsonDocument::fromJson(taskData);
    QJsonObject allTasks = taskDoc.object();

    for (const QString& key : allTasks.keys()) {
        QJsonObject taskData = allTasks[key].toObject();
        if (taskData["managerId"].toString() == firebase.m_uid) {
            Task w;
            w.setTaskID(taskData["taskID"].toInt());
            w.setTaskInstructions(taskData["instructions"].toString().toStdString());
            w.setComments(taskData["comments"].toString().toStdString());
            w.setPartsNeeded(taskData["parts"].toString().toStdString());
            w.setTaskStatus(w.statusFromString(taskData["status"].toString().toStdString()));
            w.setTaskPriority(w.priorityFromString(taskData["priority"].toString().toStdString()));
            w.setTaskWorkerID(taskData["workerId"].toInt());
            w.setFirebaseKey(key.toStdString());
            w.setWorkerFirebaseKey(taskData["workerFirebaseId"].toString().toStdString());
            w.setCarFirebaseKey(taskData["carFirebaseId"].toString().toStdString());
            for (auto c : cars)
            {
                if (w.getCarFirebaseKey() == c.getFirebaseKey())
                {
                    w.setTaskCar(c);
                    break;
                }
...
void AutoMagik::addTask()
{
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        //firebase worker id
        std::string workerFBid;
        if (!workers.empty() && workerComboBox->currentIndex() >= 0) {
            workerFBid = workers[workerComboBox->currentIndex()].getFirebaseKey();
        }
        newTask.setWorkerFirebaseKey(workerFBid);

        std::string carFBid;
        if (!cars.empty() && carSelection->currentIndex() >= 0) {
            carFBid = cars[carSelection->currentIndex()].getFirebaseKey();
        }
        newTask.setCarFirebaseKey(carFBid);


        // Add to local list
        tasks.push_back(newTask);
        int index = static_cast<int>(tasks.size()) - 1;
        updateManagerTables();
        updateWorkerDashboard();

        // Connect to Firebase signal before sending
        connect(&firebase, &Firebase::taskAdded, this, [this, taskId = newTask.getTaskID()](const QString& fbKey) {
            auto it = std::find_if(tasks.begin(), tasks.end(),
            [taskId](const Task& t) { return t.getTaskID() == taskId; });
        if (it != tasks.end()) {
            it->setFirebaseKey(fbKey.toStdString());
            updateManagerTables();
        }
            });
        ...
        firebase.addTaskToDatabase(
            ...
            firebase.getIdToken(),
            workerName,
            QString::fromStdString(workerFBid),
            QString::fromStdString(carFBid),
            newTask.getTaskID()
        );
    }
}
